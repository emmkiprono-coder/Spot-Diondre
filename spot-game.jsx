...
export default function SpotGame() {
  ...
  const handleScore = (playerId, venueId) => {
  ...
  const leader = scores.emmanuel > scores.manwell ? "emmanuel"
    : scores.manwell > scores.emmanuel ? "manwell"
    : null;
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ fontSize: 13, color: "#5588BB", marginTop: 4 }}>
          Duke & Hornets Games · {total} spots total
        </div>
      </div>
          ...
          {VENUES.map(venue => (
            <div key={venue.id} style={{ marginBottom: 20 }}>
              <div style={{
                ...
                padding: "8px 14px", borderRadius: "0 12px 12px 0",
                marginBottom: 10,
              }}>
                <span style={{ fontSize: 20 }}>{venue.emoji}</span>
                <div>
                <div style={{ fontWeight: 700, fontSize: 14, letterSpacing: 0.5 }}>{venue.label}</div>
